...
(function($) {
    ...
    m_chart_chartjs_admin.init = function() {
        ...
        if (
            "default" === m_chart_admin.performance &&
            "yes" === m_chart_admin.image_support
        ) {
    ...
    m_chart_chartjs_admin.handle_chart_type = function(event) {
        ...
        if (
            ...
            $chart_meta_box
                .find(".row.vertical-axis, .row.horizontal-axis, .row.y-min")
                .addClass("hide");
            $chart_meta_box.find(".row.two").removeClass("show-shared");
        }

        if ("scatter" === chart_type || "bubble" === chart_type) {
            $chart_meta_box.find(".row.y-min").addClass("hide");
            $chart_meta_box.find(".row.two").removeClass("show-shared");
        ...
        if ("radar" === chart_type || "radar-area" === chart_type) {
            $chart_meta_box
                .find(".row.vertical-axis, .row.horizontal-axis, .row.y-min")
                .addClass("hide");
            $spreadsheet_tabs.removeClass("hide");
    ...
    m_chart_chartjs_admin.generate_image_from_chart = function(event) {
        ...
        var $canvas_source = document.getElementById(
            "m-chart-" + event.post_id + "-" + event.instance
        );
        var $target_canvas = $("#m-chart-canvas-render-" + event.post_id);
        var target_context = document
            .getElementById("m-chart-canvas-render-" + event.post_id)
            .getContext("2d");
        ...
        var $chart_context = window[
            "m_chart_chartjs_" + event.post_id + "_1"
        ].chart.canvas.getContext("2d");

        $chart_context.save();
        $chart_context.globalCompositeOperation = "destination-over";
        $chart_context.fillStyle = "white";
        $chart_context.fillRect(
            0,
            0,
            window["m_chart_chartjs_" + event.post_id + "_1"].chart.width,
            window["m_chart_chartjs_" + event.post_id + "_1"].chart.height
        );
    ...
    m_chart_chartjs_admin.refresh_chart = function(event) {
        ...
        window["m_chart_chartjs_" + m_chart_admin.post_id + "_1"].chart.data =
            event.response.data.data;
        window[
            "m_chart_chartjs_" + m_chart_admin.post_id + "_1"
        ].chart.config.type = event.response.data.type;
        window[
            "m_chart_chartjs_" + m_chart_admin.post_id + "_1"
        ].chart.options = event.response.data.options;

        window[
            "m_chart_chartjs_" + m_chart_admin.post_id + "_1"
        ].chart_args.value_prefix = event.response.data.value_prefix;
        window[
            "m_chart_chartjs_" + m_chart_admin.post_id + "_1"
        ].chart_args.value_suffix = event.response.data.value_suffix;
        ...
        window[
            ...
            onComplete: function() {
                if (
                    false ===
                    window["m_chart_chartjs_" + m_chart_admin.post_id + "_1"]
                        .render_1
                ) {
                    return;
                }
