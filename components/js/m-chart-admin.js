(function($) {
    ...
    m_chart_admin.init = function() {
        ...
        this.$setting_inputs = $(document.getElementById("m-chart")).find(
            ".settings input, .settings select"
        );
        this.$title_input = $(document.getElementById("titlewrap")).find(
            "input"
        );

        this.$subtitle_input = $(document.getElementById("titlediv")).find(
            "#m-chart-subtitle"
        );
        ...
        this.$form_buttons = $(
            "#save-post, #wp-preview, #post-preview, #publish"
        );
        ...
        $form.on("submit", function(event) {
            if (false === m_chart_admin.allow_form_submission) {
                ...
                $(document.getElementById("m-chart-spreadsheet"))
                    .find(".data")
                    .val(JSON.stringify(m_chart_admin.get_data()));
    ...
    m_chart_admin.build_spreadsheets = function() {
        this.$spreadsheet_divs = $(
            document.getElementById("hands-on-table-sheets")
        );
        this.$spreadsheet_tabs = $(
            document.getElementById("hands-on-table-sheet-tabs")
        );
        this.sheet_div_template = Handlebars.compile(
            $(document.getElementById("m-chart-sheet-div")).html()
        );
        this.sheet_tab_template = Handlebars.compile(
            $(document.getElementById("m-chart-sheet-tab")).html()
        );
    ...
    m_chart_admin.create_spreadsheet = function(i, data) {
        this.$spreadsheet_divs.append(
            this.sheet_div_template({ post_id: this.post_id, instance: i })
        );
        // Note we're purposely not getting a jQuery version of this object because handsontable likes it that way
        var $spreadsheet_div = document.getElementById(
            "hands-on-table-sheet-" + this.post_id + "-" + i
        );
        ...
        var $tab_input = $(
            "#hands-on-table-sheet-tab-" + this.post_id + "-" + i + " input"
        );
    ...
    m_chart_admin.handle_sheet_controls = function() {
        ...
        this.$spreadsheet_tabs.find(".add-sheet").on("click", function(event) {
            ...
            var new_tab = document.getElementById(
                "hands-on-table-sheet-tab-" +
                    m_chart_admin.post_id +
                    "-" +
                    m_chart_admin.last_set
            );
        ...
        this.$spreadsheet_tabs.on("click", ".nav-tab", function(event) {
            ...
            m_chart_admin.$spreadsheet_divs
                .find(".hands-on-table-sheet")
                .addClass("hide");
            $(
                document.getElementById(
                    "hands-on-table-sheet-" +
                        m_chart_admin.post_id +
                        "-" +
                        $(this).data("instance")
                )
            ...
            m_chart_admin.$spreadsheet_tabs
                .find(".nav-tab")
        ...
        this.$spreadsheet_tabs.on("dblclick taphold", ".nav-tab", function(
            event
        ) {
            event.preventDefault();
        ...
        this.$spreadsheet_tabs.on("click", ".dashicons-dismiss", function(
            event
        ) {
    ...
    m_chart_admin.resize_input = function($input) {
        ...
        var font = window.getComputedStyle(
            document.getElementById($input.attr("id"))
        ).font;
        var text = $input.val();

        // Get what we need to properly size the input with the value width
        var border_width = window
            .getComputedStyle(document.getElementById($input.attr("id")))
        ...
        $input.css(
            "width",
            border_width * 2 +
                parseInt(padding_left) +
                width +
                parseInt(padding_right) +
                "px"
        );
    };
    ...
    m_chart_admin.handle_csv_import = function() {
        ...
        var $import_form = $(
            document.getElementById("m-chart-csv-import-form")
        );
        ...
        $import_form.on("submit", function(event) {
            ...
            request.done(function(response) {
                ...
                m_chart_admin.$spreadsheets[m_chart_admin.active_set].loadData(
                    response.data
                );
    ...
    m_chart_admin.handle_csv_export = function() {
        $(document.getElementById("m-chart-csv"))
            .find(".export a")
            .on("click", function(event) {
                event.preventDefault();

                var $form = $(
                    document.getElementById("m-chart-csv-export-form")
                );
                var $data = m_chart_admin.$spreadsheets[
                    m_chart_admin.active_set
                ].getData();

                var set_name = m_chart_admin.$spreadsheet_tabs
                    .find(".nav-tab-active input")
                    .val();

                $(document.getElementById("m-chart-csv-post-id")).val(
                    m_chart_admin.post_id
                );
                $(document.getElementById("m-chart-csv-data")).val(
                    JSON.stringify($data)
                );
                $(document.getElementById("m-chart-csv-title")).val(
                    m_chart_admin.$title_input.val()
                );
                $(document.getElementById("m-chart-csv-set-name")).val(
                    set_name
                );
    ...
    m_chart_admin.refresh_chart = function() {
        if (
            "no-preview" === this.performance ||
            "yes" !== this.instant_preview_support
        ) {
            return false;
        }
