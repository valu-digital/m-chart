...
class M_Chart {
	...
	public function init() {
		...
		add_filter( 'script_loader_tag', [ $this, 'add_attribute_to_script' ], 10, 3 );
	...
	public function add_attribute_to_script( $tag, $handle, $src ) {
		if ( "chartjs" === $handle ) {
	...
	public function get_chart( $post_id = null, $args = array() ) {
		...
		if ( 'image' == $args['show'] || is_feed() || $this->is_shortcake || $this->is_amp_endpoint() ) {
			...
			if ( $this->is_amp_endpoint() ) {
				ob_start();
				?>
			<amp-img src="<?php echo esc_url( $image['url'] ); ?>" alt="<?php echo esc_attr( $image['name'] ); ?>" width="<?php echo absint( $image['width'] ); ?>" height="<?php echo absint( $image['height'] ); ?>"
			         class="<?php echo esc_attr( $classes ); ?>"></amp-img><?php
				return ob_get_clean();
			} else {
				ob_start();
				?><img src="<?php echo esc_url( $image['url'] ); ?>" alt="<?php echo esc_attr( $image['name'] ); ?>" width="<?php echo absint( $image['width'] ); ?>" height="<?php echo absint( $image['height'] ); ?>"
				       alt="<?php echo esc_attr( get_the_title( $post_id ) ); ?>" class="<?php echo esc_attr( $classes ); ?>" /><?php
				return ob_get_clean();
			}
		}
	...
	public function get_chart_iframe( $post_id, $args = array() ) {
		...
		ob_start();
		?>
	<iframe id="m-chart-container-<?php echo absint( $post_id ); ?>-<?php echo absint( $this->instance ); ?>" class="m-chart-iframe" width="100%" height="<?php echo absint( $post_meta['height'] + 1 ); ?>"
	        src="<?php echo esc_url_raw( $src_url ); ?>" frameborder="0"></iframe><?php
		if ( 'show' == $args['share'] ) {
			unset( $args['share'] );
