<table class="<?php echo esc_attr( $classes ); ?>">
	...
	if ( isset( m_chart()->parse()->value_labels['first_row'] ) ) {
		...
		?>
		<tr>
			<th colspan="<?php echo count( $first_row ) + 1; ?>"><?php echo get_the_title( $post_id ) . $set_name; ?></th>
		</tr>
		<tr>
			<th></th>
			<?php
		...
		?>
		<tr>
			<th colspan="<?php echo count( $first_row ); ?>"><?php echo get_the_title( $post_id ) . $set_name; ?></th>
		</tr>
		<tr>
			<?php
			foreach ( $first_row as $label ) {
